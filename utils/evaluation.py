...
def get_siamese_data(encoder, fold):
    x, y, x_ts, y_ts, y_cv = list(), list(), list(), list(), list()
    for writer in range(CONFIG.wrt_cnt):
        ...
        if (fold < 0 and writer >= CONFIG.tr_wrt_cnt) or (0 <= fold == writer // (CONFIG.wrt_cnt // CONFIG.spt_cnt)):
            ...
            if CONFIG.sms_use_ref_smp:
                ...
                y_ts.extend(np.ones((CONFIG.ref_smp_cnt, 1)))
            ...
            y_cv.extend(np.ones((CONFIG.ref_smp_cnt * len(encoded_genuine[CONFIG.ref_smp_cnt:]), 1)))

            x_ts.extend(map(lambda z: np.array(z, ndmin=3), itertools.product(
                encoded_genuine[:CONFIG.ref_smp_cnt], encoded_forgery
            )))
            y_ts.extend(np.zeros((len(encoded_forgery), 1)))
            y_cv.extend(np.ones((CONFIG.ref_smp_cnt * len(encoded_forgery), 1)))
        else:
    ...
    return x, y, x_ts, y_ts, y_cv


def get_evaluation(x, y, x_ts, y_ts, y_cv, fold):
    sms = SiameseClassifier(fold)
    if CONFIG.sms_md == 'train':
        sms.fit(x, y, x_ts, y_cv)
    ...
    cm = confusion_matrix(y_true=y_ts, y_pred=y_prd)
    scores.extend([(cm[0][0] + cm[1][1]) / np.sum(cm), cm[1][0] / np.sum(cm[1]), cm[0][1] / np.sum(cm[0])])
